...
test_that("ol_read object fallback respects ref", {
  tmpdir <- withr::local_tempdir()
  old_opt <- getOption("ol.root")
  on.exit(options(ol.root = old_opt), add = TRUE)
  ...
  ol_save("versioned_object", list(v = 1))
  Sys.sleep(0.1)
  ol_save("versioned_object", list(v = 2))
  ol_tag_object("versioned_object", "v1", when = "first")

  first <- ol_read("versioned_object", ref = "@v1")
  latest <- ol_read("versioned_object", ref = "@latest")

  expect_equal(first$v, 1)
  expect_equal(latest$v, 2)
})
...
test_that("ol_read collect=FALSE errors clearly for object-only names", {
  tmpdir <- withr::local_tempdir()
  old_opt <- getOption("ol.root")
  on.exit(options(ol.root = old_opt), add = TRUE)
  ...
  ol_save("object_only", list(v = 1))
  expect_error(
    ol_read("object_only", collect = FALSE),
