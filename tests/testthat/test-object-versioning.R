...
test_that("ol_tag_object validates inputs", {
  ...
  expect_error(ol_tag_object("test_obj", "tag2", when = "missing_version"), "Version not found")
})

test_that("ol_tag_object accepts explicit existing version timestamp", {
  tmpdir <- withr::local_tempdir()
  old_opt <- getOption("ol.root")
  on.exit(options(ol.root = old_opt), add = TRUE)

  options(ol.root = tmpdir)
  ol_init("test_explicit_when")

  ol_save("obj", list(v = 1))
  Sys.sleep(0.1)
  ol_save("obj", list(v = 2))

  state <- OmicsLake:::.ol_get_backend_state("test_explicit_when")
  ts <- DBI::dbGetQuery(
    state$conn,
    "SELECT CAST(version_ts AS VARCHAR) AS v FROM ol.__ol_objects WHERE name = 'obj' ORDER BY version_ts ASC LIMIT 1"
  )$v[[1]]

  expect_silent(ol_tag_object("obj", "first_explicit", when = ts))
  tagged <- ol_read_object("obj", ref = "@first_explicit")
  expect_equal(tagged$v, 1)
})
