...
test_that("QueryBuilder validates table names and numeric bounds", {
  lake <- Lake$new("test_qb_validation")
  lake$put("data", data.frame(x = 1:5))

  expect_error(
    lake$from("missing_table"),
  ...
  expect_error(
    lake$from("data")$join("missing_table"),
    "Unknown join table 'missing_table'"
  )

  expect_error(
    lake$from("data")$join("data", on = 1),
    "on must be a non-empty character vector"
  )

  expect_error(
    lake$from("data")$limit(0),
    "n must be > 0"
  )

  expect_error(
    lake$from("data")$offset(-1),
    "n must be >= 0"
  )

  expect_error(
    lake$from("data")$top(-3, by = x),
    "n must be > 0"
  )

  unlink(file.path(path.expand("~"), ".omicslake", "test_qb_validation"), recursive = TRUE)
})
