...
test_that("ol_tag can snapshot from a referenced tag", {
  tmpdir <- withr::local_tempdir()
  old_opt <- getOption("ol.root")
  on.exit(options(ol.root = old_opt), add = TRUE)
  ...
  ol_write("test_table", data.frame(x = 10:12), mode = "overwrite")
  ol_tag("test_table", "from_v1", ref = "@v1.0")

  from_v1 <- ol_read("test_table", ref = "@from_v1")
  expect_equal(from_v1$x, 1:3)
})
