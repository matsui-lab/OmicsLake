...
Lake <- R6::R6Class("Lake",
  ...
  public = list(
    ...
    tag = function(name, tag) {
      ...
      if (!is.null(adapter_info) || !is.null(adapter)) {
        ...
        if (!is.null(adapter)) {
          # Use adapter to get components and tag them
          comp_df <- adapter$components(self, name)
            ...
            for (i in seq_len(nrow(comp_df))) {
              ...
              tryCatch({
                if (identical(comp_type, "table")) {
                  ol_tag(comp_name, tag, project = private$.project)
                } else {
    ...
    log = function(name = NULL, n = 20) {
      if (is.null(name)) {
        return(ol_log_commits(project = private$.project, n = n))
      }
    ...
    drop = function(name, force = FALSE) {
      ...
      # Drop as table or object based on what exists
      if (private$.is_table(name)) {
    ...
    #' @description Check whether a data name exists in the lake
    #' @param name Data name
    #' @param type One of "any", "table", or "object"
    #' @return TRUE/FALSE
    exists = function(name, type = c("any", "table", "object")) {
      type <- match.arg(type)
      in_tables <- private$.is_table(name)
      in_objects <- private$.is_object(name)
      # Also check if it's an adapter-managed object
      in_adapter <- if (!in_tables && !in_objects) {
        !is.null(private$.find_adapter_for_stored(name))
      } else {
        FALSE
      }
      switch(type,
        "any" = in_tables || in_objects || in_adapter,
        "table" = in_tables,
        "object" = in_objects || in_adapter
      )
    },

    #' @description Find data names matching a pattern
    #' @param pattern Optional regex pattern; NULL returns all names
    #' @param type One of "any", "table", or "object"
    #' @param ignore.case Whether matching should ignore case
    #' @return Data frame with columns name, type
    find = function(pattern = NULL, type = c("any", "table", "object"),
                    ignore.case = TRUE, fixed = FALSE, fuzzy = TRUE,
                    max_distance = 3L, min_score = -Inf,
                    prefer_type = c("none", "table", "object"), limit = Inf) {
      type <- match.arg(type)
      ...
      if (nrow(all_items) > 0) {
        ...
        if (length(adapter_names) > 0) {
          all_items <- rbind(all_items, build(adapter_names, "object"))
        }
        keep <- !grepl(internal_pattern, all_items$name)
        all_items <- all_items[keep, , drop = FALSE]
        all_items <- all_items[!duplicated(all_items$name), , drop = FALSE]
      }
      if (!nrow(all_items)) return(all_items)
      ...
      if (is.finite(min_score) && nrow(all_items) > 0) {
        all_items <- all_items[all_items$score >= min_score, , drop = FALSE]
      }
      ...
      rownames(all_items) <- NULL
      all_items
    },

    #' @description One-line status summary for this lake
    #' @return A one-row data frame with status fields
    status = function() {
      n_tables <- tryCatch(nrow(self$tables()), error = function(e) 0L)
      n_objects <- tryCatch(nrow(self$objects()), error = function(e) 0L)
      n_tags <- tryCatch(nrow(ol_list_tags(project = private$.project)),
        error = function(e) 0L)
      shortcuts_ok <- tryCatch(
        identical(getOption("ol.project"), private$.project),
        error = function(e) FALSE)
      git_dirty <- tryCatch({
        out <- system2("git", c("status", "--porcelain"),
      ...
      data.frame(
        ...
        objects = n_objects,
        tags = n_tags,
        default_shortcuts = shortcuts_ok,
        git_dirty = git_dirty,
    ...
    doctor = function(verbose = TRUE) {
      ...
      checks[[2L]] <- list(check = "tables readable", ok = c2,
        detail = if (c2) "Table listing succeeded" else "Failed to list tables",
      ...
      if (is.na(g)) {
        gd <- "Not a git repository or git not available"; gf <- ""; g <- TRUE
      } else {
        gd <- if (g) "Working tree is clean" else "Uncommitted changes detected"
        gf <- if (g) "" else "git add . && git commit -m 'checkpoint'"
      }
    ...
    #' @description Run the repair workflow on this lake
    #' @param ... Arguments forwarded to the repair implementation
    #' @return Repair report (invisibly)
    repair = function(...) {
      .ol_run_lake_repair(lake = self, project = private$.project, ...)
    },

    #' @description Clean up connections when object is garbage collected
    finalize = function() {
      # Note: we don't disconnect here because the connection is managed
      # by the backend registry and may be shared
      invisible(NULL)
    }
  ),
  ...
  private = list(
    ...
    .adapter_prefix = function(name, adapter_name) {
      prefix_map <- c(
        "SummarizedExperiment" = "__se__",
        "SingleCellExperiment" = "__sce__",
        "MultiAssayExperiment" = "__mae__",
        "Spectra"              = "__spectra__",
        "QFeatures"            = "__qfeatures__",
        "MsExperiment"         = "__mse__",
        "xcms"                 = "__xcms__",
        "Chromatograms"        = "__chrom__")
      ...
      paste0(name, ".__", tolower(adapter_name), "__.")
    },
...
# S3 method wrappers for bracket notation on Lake objects
#' @export
`[.Lake` <- function(x, name, i, j) {
  if (missing(i) && missing(j)) {
    return(x$get(name))
  }
  data <- x$get(name)
