...
wrap_fn <- function(fn, lake, output_name, input_names = NULL,
    ...
    function(...) {
        args <- list(...)
        deps <- .ol_collect_wrap_dependencies(args, input_names = input_names)
...
wrap_call <- function(lake, fn, ..., output = NULL, save = TRUE) {
    args <- list(...)
    deps <- .ol_collect_wrap_dependencies(args)
    result <- do.call(fn, args)
...
mark <- function(name, data = NULL, lake = NULL) {
    ...
    metadata <- list(
        ...
        dims = if (!is.null(data) && (is.data.frame(data) || 
            is.matrix(data))) dim(data) else NULL,
...
.ol_marker_file_metadata <- function(path,
    ...
    if (!is.null(info) && nrow(info) > 0) {
        ...
        mtime <- info$mtime[[1]]
        if (!is.na(mtime)) {
            out$mtime_utc <- format(as.POSIXct(mtime, tz = "UTC"),
                "%Y-%m-%d %H:%M:%S UTC")
        }
    }

    max_bytes <- getOption("ol.external.hash.max_bytes", 50 * 1024 * 1024)
    if (!is.numeric(max_bytes) || length(max_bytes) != 1 || is.na(max_bytes) || 
        max_bytes < 0) {
...
.ol_marker_apply_name_hint <- function(out, name) {
...
.ol_marker_pick_source_id <- function(data) {
    source_id <- .ol_marker_scalar_chr(data$source_id, default = NA_character_)
    if (!is.na(source_id)) {
        return(source_id)
    }
    keys <- c("path", "file", "url", "endpoint", "uri")
    for (key in keys) {
        source_id <- .ol_marker_scalar_chr(data[[key]], default = NA_character_)
        if (!is.na(source_id)) {
            return(source_id)
        }
    }
...
.ol_marker_apply_list_source <- function(out, data) {
    source_kind <- .ol_marker_scalar_chr(data$source_kind,
        default = NA_character_)
    source_id <- .ol_marker_pick_source_id(data)
    ...
    if (identical(out$source_kind, "value")) {
        if (is.character(out$source_id) &&
            nzchar(out$source_id) &&
            grepl("^(https?|ftp)://", out$source_id, ignore.case = TRUE)) {
            out$source_kind <- "url"
        } else if (is.character(out$source_id) &&
            nzchar(out$source_id) &&
            file.exists(out$source_id)) {
            out$source_kind <- "file"
        }
    }
    if (identical(out$source_kind, "file") &&
        is.character(out$source_id) &&
        nzchar(out$source_id)) {
...
.ol_marker_apply_optional_fields <- function(out, data) {
    ...
    content_md5 <- .ol_marker_scalar_chr(data$content_md5,
        default = NA_character_)
    ...
    last_modified <- .ol_marker_scalar_chr(data$last_modified,
        default = NA_character_)
    if (!is.na(last_modified)) {
        out$last_modified <- last_modified
    }
    status_code <- .ol_marker_scalar_int(data$status_code,
        default = NA_integer_)
    if (!is.na(status_code)) {
        out$status_code <- status_code
    }
    response_md5 <- .ol_marker_scalar_chr(data$response_md5,
        default = NA_character_)
...
.ol_marker_source_metadata <- function(data, name = NULL) {
    out <- .ol_marker_empty_source_metadata()
    out <- .ol_marker_apply_name_hint(out, name)
    if (is.null(data)) {
        return(out)
    }
    if (is.list(data) && !is.data.frame(data) && !is.null(names(data))) {
        out <- .ol_marker_apply_list_source(out, data)
        return(.ol_marker_apply_optional_fields(out, data))
    }
...
link <- function(from, to, lake = NULL, relationship = "linked") {
    ...
    # Determine types
    is_object <- lake$.__enclos_env__$private$.is_object
    from_type <- if (is_object(from)) "object" else "table"
    to_type <- if (is_object(to)) "object" else "table"
    ...
    public = list(
        ...
        print = function() {
            ...
            for (i in seq_along(private$.steps)) {
                lines <- c(lines, paste0("  ", i, ". ",
                    private$.steps[[i]]$name))
            }
            writeLines(lines)
            invisible(self)
        }
...
.ol_trace_install <- function(functions, record_fun, env) {
    for (fn_name in names(functions)) {
        ...
        tracer <- bquote({
            .(record_fun)(.(fn_name), .(role), match.call())
        })
        tryCatch(
            trace(fn_name, tracer, print = FALSE, where = env),
...
.ol_trace_remove <- function(functions, env) {
    for (fn_name in names(functions)) {
        tryCatch(untrace(fn_name, where = env), error = function(e) NULL)
    }
    invisible(TRUE)
}
...
trace_calls <- function(functions, expr, lake = NULL) {
    ...
    record_fun <- function(fn_name, role, call) {
    ...
    .ol_trace_install(functions, record_fun, caller_env)
    result <- tryCatch(
        eval(substitute(expr), envir = caller_env),
        finally = .ol_trace_remove(functions, caller_env)
    )
    list(result = result, calls = calls_env$calls)
}
