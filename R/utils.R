.ol_root <- function() getOption("ol.root", file.path(path.expand("~"),
    ".omicslake"))
...
.ol_validate_name <- function(name, type = "name") {
    ...
    if (grepl("['\";`]", name)) {
        warning(type, " '", name,
            "' contains special SQL characters which may cause issues",
            call. = FALSE)
    }
...
.ol_validate_data_frame <- function(data) {
    ...
    if (any(list_cols)) {
        warning(
            "list columns may not be supported by Arrow/Parquet: ",
