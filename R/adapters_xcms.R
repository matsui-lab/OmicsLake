#' @title XCMS Adapter
#' @description Adapter for storing and retrieving xcms objects
#' (\code{XCMSnExp} and \code{XcmsExperiment}).
#'
#' @details
...
XCMSAdapter <- R6::R6Class("XCMSAdapter",
    ...
    public = list(
        ...
        can_handle = function(data) {
            inherits(data, "XCMSnExp") || inherits(data, "XcmsExperiment")
        },
        priority = function() {
            121
        },
        ...
        get = function(lake, name, ref = "@latest") {
            ...
            tryCatch(
                ol_read_object(paste0(prefix, "object"), ref = ref,
                    project = project),
                error = function(e) {
                    stop("Failed to restore xcms object for '", name, "'",
                        call. = FALSE)
                }
            )
        },
        ...
        list_names = function(lake) {
            ...
            manifest_pattern <- "\\.__xcms__\\.manifest$"
        ...
        .load_manifest = function(prefix, name, ref, project) {
            tryCatch(
                ol_read_object(paste0(prefix, "manifest"), ref = ref,
                    project = project),
                error = function(e) {
                    stop("Cannot find xcms manifest for '", name, "'",
                        call. = FALSE)
                }
            )
        }
