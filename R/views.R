...
#' creating reusable queries, especially for comparing multiple versions of
#' data.
#' Views do not store data but execute their query each time they are
#' referenced.
#'
...
#' @param replace Whether to replace the view if it already exists (default:
#' TRUE)
...
.ol_view_dependencies <- function(conn, ident_deps, view_name) {
    deps_query <- sprintf(
        paste(
            "SELECT parent_name FROM %s WHERE child_name = %s",
            "AND child_type = 'view' ORDER BY created_at"
        ),
        ident_deps,
        DBI::dbQuoteString(conn, view_name)
    )
...
ol_list_views <- function(project = getOption("ol.project")) {
    ...
    views <- tryCatch(
        ...
        error = function(e) .ol_empty_views_df()
    )
    if (nrow(views) == 0) {
        return(.ol_empty_views_df())
    }
    .ol_ensure_dependencies_table(state)
    ident_deps <- .ol_sql_ident(conn, state, "__ol_dependencies")
    views$dependencies <- vapply(views$view_name, function(vname) {
        .ol_view_dependencies(conn, ident_deps, vname)
    }, character(1))
    views
}
