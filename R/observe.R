...
#' (e.g. \code{read.csv()}, \code{write.csv()}, \code{readRDS()},
#' \code{saveRDS()}).
#'
#' ## Limitations
#'
...
.ol_track_mode <- function(fun_name) {
    ...
    if (grepl("^(read|load|fread)", fun_name)) {
        return("read")
    }
    if (grepl("^(write|save|fwrite)", fun_name)) {
        return("write")
    }
    NA_character_
}
...
.record_read <- function(path) {
    if (.is_observing()) {
        ...
        .observe_env$events <- append(.observe_env$events,
            list(list(type = "read", path = norm_path)))
    }
}
...
.record_write <- function(path) {
    if (.is_observing()) {
        ...
        .observe_env$events <- append(.observe_env$events,
            list(list(type = "write", path = norm_path)))
    }
}
...
.ol_observed_node_names <- function(paths, prefix = "file:") {
    ...
    dup_bases <- unique(bases[duplicated(bases) | duplicated(bases,
        fromLast = TRUE)])
...
observe <- function(expr, track_functions = NULL) {
    ...
    if (length(track_functions) > 0) {
        for (fun_name in unique(track_functions)) {
            ...
            original <- tryCatch(get(fun_name, mode = "function",
                inherits = TRUE), error = function(e) NULL)
            if (is.null(original)) {
                next
            }
            assign(fun_name, .ol_make_io_wrapper(fun_name, fun_mode, original),
                envir = eval_env)
        }
    }
    result <- tryCatch(
...
print.lake_observation <- function(x, ...) {
    ...
    if (nrow(x$lineage) > 0) {
        ...
        for (i in seq_len(nrow(x$lineage))) {
            lines <- c(lines, paste0("  ", x$lineage$parent[i], " -> ",
                x$lineage$child[i]))
        }
    }
    writeLines(lines)
...
.ol_track_validate_bool <- function(x, arg) {
    if (!is.logical(x) || length(x) != 1 || is.na(x)) {
        stop(arg, " must be TRUE or FALSE", call. = FALSE)
    }
}

.ol_track_resolve_target_lake <- function(project, extra) {
    if (is.null(project)) {
        if (length(extra) > 0) {
...
.ol_track_resolve_functions <- function(track_functions) {
    if (is.null(track_functions)) {
        track_functions <- .ol_default_track_functions()
    }
    if (!is.character(track_functions)) {
...
.ol_track_install_wrappers <- function(track_functions, g_env) {
    wrappers <- list()
    for (fun_name in unique(track_functions)) {
        fun_mode <- .ol_track_mode(fun_name)
        if (is.na(fun_mode)) {
        ...
        old_global <- if (had_global) {
            get(fun_name, envir = g_env, inherits = FALSE)
        } else {
            NULL
        }
        assign(fun_name, .ol_make_io_wrapper(fun_name, fun_mode, original),
...
.ol_track_install_last_hook <- function(g_env) {
    if (exists(".ol_prev_last_fn", envir = g_env, inherits = FALSE)) {
        rm(list = ".ol_prev_last_fn", envir = g_env)
    }
    assign(".ol_prev_last_fn", get0(".Last", envir = g_env, inherits = FALSE),
        envir = g_env)
    assign(
        ".Last",
        function() {
            try(OmicsLake::ol_disable_transparent_tracking(commit = TRUE),
                silent = TRUE)
            old_last <- get0(".ol_prev_last_fn", envir = .ol_global_env(),
                inherits = FALSE)
            if (exists(".ol_prev_last_fn", envir = .ol_global_env(),
                inherits = FALSE)) {
...
.ol_track_empty_observation <- function() {
    structure(
        list(
            result = NULL,
            reads = character(0),
...
.ol_track_current_observation <- function() {
    structure(
        list(
            result = NULL,
            reads = .observe_env$reads,
...
.ol_track_restore_wrappers <- function(wrappers) {
    g_env <- .ol_global_env()
    for (wrapper in rev(wrappers)) {
...
.ol_track_restore_last_hook <- function() {
    g_env <- .ol_global_env()
    if (!isTRUE(.auto_track_env$installed_last_hook)) {
...
.ol_track_commit_session <- function(obs) {
    recorded <- .ol_record_observation_to_lake(
        obs,
        .auto_track_env$lake,
        prefix = .auto_track_env$prefix
    )
...
observe_to_lake <- function(expr, lake, prefix = "file:",
    ...
    obs <- eval(
        substitute(observe(EXPR, track_functions = TRACK), list(EXPR = expr_sub,
...
.ol_track_validate_enable_args <- function(
    ...
    if (!is.null(snapshot) && (
        ...
        stop("snapshot must be NULL or a non-empty character string",
            call. = FALSE)
    }
    .ol_track_validate_bool(store_observation, "store_observation")
    if (!is.null(observation_name) && (!is.character(observation_name) ||
        length(observation_name) != 1 || !nzchar(observation_name))) {
        stop("observation_name must be NULL or a non-empty character string",
            call. = FALSE)
    }
    .ol_track_validate_bool(auto_disable, "auto_disable")
    invisible(TRUE)
}

.ol_track_enable_state <- function(
    target_lake,
    wrappers,
    prefix,
    snapshot,
    ...
    .auto_track_env$observation_depends_on <- observation_depends_on
    .auto_track_env$installed_last_hook <- FALSE
    if (!isTRUE(auto_disable)) {
        return(invisible(target_lake))
    }
    .ol_track_install_last_hook(g_env)
...
ol_enable_transparent_tracking <- function(project = NULL,
    ...
    if (.auto_track_env$active) {
        stop(
            "Transparent tracking is already active.",
            "Call ol_disable_transparent_tracking() first.",
            call. = FALSE)
    }
    .ol_track_validate_enable_args(
        snapshot = snapshot,
    ...
    extra <- list(...)
    target_lake <- .ol_track_resolve_target_lake(project, extra)
    track_functions <- .ol_track_resolve_functions(track_functions)
    g_env <- .ol_global_env()
    wrappers <- .ol_track_install_wrappers(track_functions, g_env)
    observation_depends_on <- match.arg(observation_depends_on,
        c("writes", "reads", "both", "none"))
    .ol_track_enable_state(
...
#'
#' @param commit If TRUE, write tracked lineage and optional observation object
#' to Lake
#' @return A `lake_observation` object (invisible) with tracked
#' reads/writes/lineage
#' @export
#' @examples
#' if (FALSE) {
...
ol_disable_transparent_tracking <- function(commit = TRUE) {
    .ol_track_validate_bool(commit, "commit")
    if (!.auto_track_env$active) {
        return(invisible(.ol_track_empty_observation()))
    }
    obs <- .ol_track_current_observation()
    if (isTRUE(commit)) {
        .ol_track_commit_session(obs)
    }
    .ol_track_restore_wrappers(.auto_track_env$wrappers)
    while (.observe_env$depth > 0) {
        .stop_observe()
    }
    .ol_track_restore_last_hook()
    .ol_reset_auto_track_env()
    invisible(obs)
}

.ol_track_validate_pipeline_args <- function(snapshot, save_result, result_name,
    store_observation, observation_name) {
    if (!is.null(snapshot) && (
        !is.character(snapshot) ||
        length(snapshot) != 1 ||
        !nzchar(snapshot)
    ...
    .ol_track_validate_bool(save_result, "save_result")
    .ol_track_validate_bool(store_observation, "store_observation")
    if (!is.null(result_name) && (!is.character(result_name) || 
        length(result_name) != 1 || !nzchar(result_name))) {
        stop("result_name must be NULL or a non-empty character string",
            call. = FALSE)
    }
    if (!is.null(observation_name) && (!is.character(observation_name) || 
        length(observation_name) != 1 || !nzchar(observation_name))) {
...
.ol_track_observe_expr <- function(expr_sub, track_functions, env) {
    eval(
        substitute(observe(EXPR, track_functions = TRACK), list(EXPR = expr_sub,
            TRACK = track_functions)),
        envir = env
...
.ol_track_save_result <- function(lake, obs, recorded, save_result, result_name,
    result_depends_on) {
    ...
    lake$put(result_name, obs$result, depends_on = deps)
    invisible(TRUE)
}

.ol_track_save_observation <- function(lake, obs, recorded, store_observation,
    observation_name, observation_depends_on) {
    if (!isTRUE(store_observation)) {
        return(invisible(TRUE))
    }
    if (is.null(observation_name)) {
        observation_name <- .ol_default_result_name(
            prefix = "pipeline_observation"
        )
    }
...
.ol_track_script_validate <- function(path, source_args) {
    ...
    reserved <- c("file", "local", "chdir", "echo")
    dup <- intersect(names(source_args), reserved)
    if (length(dup) > 0) {
        stop("source_args cannot include reserved names: ", paste(dup,
            collapse = ", "), call. = FALSE)
    }
    normalizePath(path, winslash = "/", mustWork = TRUE)
}

.ol_track_script_default_name <- function(prefix, path) {
...
track_pipeline <- function(expr,
                            ...
                            save_result = FALSE,
                            result_name = NULL,
                            result_depends_on = c("writes", "reads", "both",
                                "none"),
                            store_observation = FALSE,
                            observation_name = NULL,
                            observation_depends_on = c("writes", "reads", 
                                "both",
                                "none"),
                            ...) {
    .ol_track_validate_pipeline_args(
        snapshot,
        save_result,
        result_name,
        store_observation,
    ...
    target_lake <- .ol_track_resolve_target_lake(project, extra)
    expr_sub <- substitute(expr)
    obs <- .ol_track_observe_expr(expr_sub, track_functions, parent.frame())
    recorded <- .ol_record_observation_to_lake(obs, target_lake,
        prefix = prefix)
    .ol_track_save_result(
        target_lake,
        obs,
        recorded,
        save_result,
...
track_script <- function(path,
                        ...
                        store_observation = FALSE,
                        observation_name = NULL,
                        observation_depends_on = c("writes", "reads", "both",
                            "none"),
                        chdir = FALSE,
                        echo = FALSE,
                        source_args = list(),
                        ...) {
    path <- .ol_track_script_validate(path, source_args)
    if (isTRUE(save_result) && is.null(result_name)) {
        result_name <- .ol_track_script_default_name("script_result_", path)
    }
    if (isTRUE(store_observation) && is.null(observation_name)) {
        observation_name <- .ol_track_script_default_name("script_observation_",
            path)
    }
    ...
    public = list(
        ...
        stop = function() {
            ...
            list(
                ...
                duration = difftime(private$.end_time, private$.start_time,
                    units = "secs"),
