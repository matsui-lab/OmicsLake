...
.ol_repro_path <- function() {
  ...
  path <- if (!is.null(opt) && is.character(opt) && length(opt) == 1 && nzchar(opt)) {
    opt
  } else {
    getwd()
  }
...
.ol_collect_git_context <- function(path) {
  ...
  status_res <- .ol_git_exec(root, c("status", "--porcelain", "--untracked-files=normal"))
  ...
  list(
    found = TRUE,
    root = root,
    branch = if (isTRUE(branch_res$ok) && nzchar(branch_res$output)) branch_res$output else NA_character_,
    commit = if (isTRUE(commit_res$ok) && nzchar(commit_res$output)) commit_res$output else NA_character_,
    commit_short = if (isTRUE(commit_res$ok) && nzchar(commit_res$output)) substr(commit_res$output, 1L, 12L) else NA_character_,
    dirty = length(status_lines) > 0,
    changed_files = as.integer(length(status_lines)),
    remote_origin = if (isTRUE(remote_res$ok) && nzchar(remote_res$output)) remote_res$output else NA_character_
  )
}
...
.ol_collect_renv_context <- function(path) {
  ...
  md5 <- tryCatch(as.character(tools::md5sum(lockfile)[[1]]), error = function(e) NA_character_)
  ...
  if (!is.null(info) && nrow(info) > 0) {
    out$lockfile_size <- as.integer(info$size[[1]])
    out$lockfile_mtime_utc <- .ol_format_time_utc(info$mtime[[1]])
  }
  ...
  out$format_version <- if (!is.null(parsed$Version)) as.character(parsed$Version) else NA_character_
  out$r_version <- if (!is.null(parsed$R$Version)) as.character(parsed$R$Version) else NA_character_
  out$packages_n <- if (is.list(parsed$Packages)) as.integer(length(parsed$Packages)) else NA_integer_
  out$renv_version <- if (!is.null(parsed$Packages$renv$Version)) as.character(parsed$Packages$renv$Version) else NA_character_
...
.ol_collect_repro_context <- function(path = .ol_repro_path(), include = .ol_repro_include()) {
...
.ol_apply_repro_guards <- function(ctx) {
  ...
  if (is.null(git) || !isTRUE(git$found)) {
    stop(
      ...
      "Set options(ol.repro.path = '<git repo>') or disable with options(ol.repro.require_clean_git = FALSE).",
      call. = FALSE
    )
  }
  if (isTRUE(git$dirty)) {
...
.ol_option_or_env <- function(option_name, env_names = character(0), default = NA_character_) {
...
.ol_latest_project_label <- function(state) {
  ...
  query <- sprintf(
    "SELECT tag, snapshot FROM %s WHERE table_name = %s ORDER BY snapshot DESC LIMIT 1",
    ident,
...
.ol_snapshot_table_map <- function(state, label) {
  ...
  ident <- .ol_sql_ident(conn, state, "__ol_refs")
  query <- sprintf(
    "SELECT table_name, snapshot, as_of FROM %s WHERE tag = %s AND table_name != %s ORDER BY as_of DESC",
    ident,
...
.ol_table_columns <- function(state, table_name) {
  conn <- state$conn
  cols <- tryCatch(
    DBI::dbListFields(conn, DBI::Id(schema = state$namespace, table = table_name)),
...
.ol_build_snapshot_validation <- function(project, label) {
  ...
  if (is.null(previous) || is.null(previous$label) || !nzchar(previous$label)) {
    return(list(
  ...
  rows <- vector("list", length(all_tables))
  ...
  for (i in seq_along(all_tables)) {
    ...
    if (isTRUE(in_prev)) {
      ...
      prev_cols_chr <- paste(prev_cols, collapse = ",")
    } else {
    ...
    if (isTRUE(in_curr)) {
      ...
      curr_cols_chr <- paste(curr_cols, collapse = ",")
    } else {
    ...
    } else if (!is.na(prev_rows) && !is.na(curr_rows) && prev_rows != curr_rows) {
    ...
    rows[[i]] <- data.frame(
      ...
      previous_columns = prev_cols_chr,
      current_columns = curr_cols_chr,
      stringsAsFactors = FALSE
    )
  }

  table_changes <- if (length(rows) > 0) do.call(rbind, rows) else data.frame(
    table_name = character(0),
    change_type = character(0),
    in_previous = logical(0),
    in_current = logical(0),
  ...
  structural_n <- sum(table_changes$change_type %in% c("added_table", "removed_table", "schema_changed"))
...
.ol_snapshot_validation_issue_text <- function(validation) {
  previous_label <- .ol_as_scalar_character(validation$previous_label, "unknown")
  structural_changes <- .ol_as_scalar_integer(.ol_get_nested(validation, c("summary", "structural_changes")), 0L)
  row_changes <- .ol_as_scalar_integer(.ol_get_nested(validation, c("summary", "row_count_changes")), 0L)
  paste0(
    "Snapshot validation vs previous label '", previous_label, "' detected ",
...
.ol_add_repro_to_params <- function(params) {
  ...
  ctx <- tryCatch(
    .ol_collect_repro_context(path = .ol_repro_path(), include = .ol_repro_include()),
    error = function(e) e
  )
  if (inherits(ctx, "error")) {
    msg <- paste0("Failed to capture reproducibility context: ", conditionMessage(ctx))
    if (.ol_repro_strict()) {
      stop(msg, call. = FALSE)
    }
...
.ol_parse_params_json <- function(json_text) {
  if (!is.character(json_text) || length(json_text) != 1 || !nzchar(json_text)) {
    return(NULL)
  }
  tryCatch(
...
#' @param snapshot_validate_mode Snapshot validation behavior: "error", "warn", or "off"
...
ol_enable_strict_repro_mode <- function(path = getwd(),
  ...
  snapshot_validate_mode <- match.arg(snapshot_validate_mode)
  if (!is.numeric(max_tables) || length(max_tables) != 1 || is.na(max_tables) || max_tables < 1) {
  ...
  new_opts <- list(
    ...
    ol.repro.path = .ol_norm(path),
    ol.repro.include = include,
    ol.snapshot.auto_validate = TRUE,
    ol.snapshot.validate.mode = snapshot_validate_mode,
    ol.snapshot.validate.max_tables = as.integer(max_tables)
  )
