...
.ol_plot_lineage_format <- function(lineage, direction) {
    ...
    if (identical(direction, "upstream")) {
        out <- lineage[, c("child", "parent", "parent_type", "relationship")]
        names(out)[3] <- "type"
        return(out)
    }

    out <- lineage[, c("parent", "child", "child_type", "relationship")]
    names(out)[3] <- "type"
    out
}

.ol_plot_lineage_collect_both <- function(name, max_depth, project) {
    lineage_up <- ol_show_lineage(
        name,
        direction = "upstream",
        max_depth = max_depth,
        project = project
    )
    lineage_down <- ol_show_lineage(
        name,
    ...
    lineage_up <- .ol_plot_lineage_format(lineage_up, "upstream")
    lineage_down <- .ol_plot_lineage_format(lineage_down, "downstream")
    if (is.null(lineage_up) && is.null(lineage_down)) {
        return(NULL)
    }
    if (!is.null(lineage_up) && !is.null(lineage_down)) {
        return(rbind(lineage_up, lineage_down))
    }
    if (!is.null(lineage_up)) {
        return(lineage_up)
    }
    lineage_down
}

.ol_plot_lineage_collect <- function(name, direction, max_depth, project) {
    if (identical(direction, "both")) {
        return(.ol_plot_lineage_collect_both(name, max_depth, project))
    }

    lineage <- ol_show_lineage(
        name,
        direction = direction,
        max_depth = max_depth,
        project = project
    )
    .ol_plot_lineage_format(lineage, direction)
}

.ol_plot_lineage_layout <- function(layout) {
    if (identical(layout, "tree")) {
        return(igraph::layout_as_tree)
    }
    if (identical(layout, "circle")) {
        return(igraph::layout_in_circle)
    }
...
ol_plot_lineage <- function(
    ...
    if (!requireNamespace("igraph", quietly = TRUE)) {
        stop(
            "Package 'igraph' is required for visualization.",
    ...
    lineage <- .ol_plot_lineage_collect(name, direction, max_depth, project)
    if (is.null(lineage)) {
        message("No dependencies found for '", name, "'")
        return(invisible(NULL))
    }
    g <- .ol_lineage_to_igraph(lineage, name)
    layout_func <- .ol_plot_lineage_layout(layout)
    if (is.null(main)) {
