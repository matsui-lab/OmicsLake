...
#' @param drop Optional character vector of columns to drop
#' @param nrows Maximum rows to return
#' @param filter Optional filter expression (string parsed by rlang)
...
ol_fread <- function(name, ref = "@latest", select = NULL, drop = NULL,
    ...
    if (!is.null(filter)) {
        ...
        tbl <- dplyr::filter(tbl, !!expr)
    }
    if (!is.infinite(nrows)) tbl <- utils::head(tbl, nrows)
    if (isTRUE(as_tibble) && requireNamespace("tibble",
        quietly = TRUE)) tibble::as_tibble(tbl) else as.data.frame(tbl)
}
...
#' @param value_col Column name containing measurement values
#' @param project Project name
...
ol_read_se <- function(name, ref = "@latest", feature_col = "feature",
    ...
    feat <- factor(df[[feature_col]])
    samp <- factor(df[[sample_col]])
    i <- as.integer(feat)
    j <- as.integer(samp)
    x <- as.numeric(df[[value_col]])
    mat <- Matrix::sparseMatrix(i = i, j = j, x = x, dims = c(nlevels(feat),
        nlevels(samp)), dimnames = list(levels(feat), levels(samp)))
    if (backing == "memory") {
        return(
    ...
    arr <- HDF5Array::writeHDF5Array(mat, filepath = h5, name = paste0(name,
        "_counts"))
    SummarizedExperiment::SummarizedExperiment(assays = list(counts = arr))
}

#' Compose a MultiAssayExperiment
#' @param assays Named list of assay specifications
#' @param ref Reference string
#' @param project Project name
#' @param backing Backing mode: "hdf5" or "memory"
...
ol_read_mae <- function(assays, ref = "@latest",
    ...
    if (!requireNamespace("MultiAssayExperiment",
        quietly = TRUE)) stop("MultiAssayExperiment required.")
    se_list <- list()
    for (nm in names(assays)) {
        cfg <- assays[[nm]]
        se_list[[nm]] <- ol_read_se(name = (cfg$name %||% nm), ref = ref,
