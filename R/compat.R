...
# Note: The original ol_* functions are defined in other files (init.R, io.R,
# etc.)
...
.ol_migration_guide_lines <- c(
    ...
    "",
    paste(
        "The ol_* API is being deprecated in favor of a cleaner,",
        "more intuitive API."
    ),
    ...
    "",
    paste(
        "    lake$get('counts', where = ~ gene_id %like% 'MT-%' &",
        "expression > 100)"
    ),
    "",
    "BRACKET NOTATION:",
